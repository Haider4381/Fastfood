...
/**
 * OrderController
 * - Daily per-branch numbering via daily_counters (branch_id + counter_date)
 * - order_no: BR{branch}-{YYYYMMDD}-{0001}
 * - day_seq column stores simple 1,2,3...
 * - Reset renames same-day orders -> -R{id} and clears counter so next = 1
 * - Edit allowed in OPEN + KITCHEN
 * - Reopen endpoint to unlock PAID/READY/PARTIAL for edit
 */
class OrderController {
    ...
    public static function createOrder(): void {
        ...
        if (!in_array($order_type, ['TAKEAWAY','DELIVERY','DINEIN','PICKUP'], true)) $order_type = 'TAKEAWAY';
        ...
        try {
            ...
            ");
            $ins->execute([
                $branch_id, $order_no, $daySeq, $order_type,
                $customer_id, ($customer_phone ?: null),
                $user['id'], $user['id'], $notes
            ]);

            $order_id = (int)$pdo->lastInsertId();
            $pdo->commit();

            json_response(['order_id'=>$order_id, 'order_no'=>$order_no, 'day_seq'=>$daySeq]);
        } catch (Throwable $e) {
            try { $pdo->rollBack(); } catch (Throwable $_) {}
    ...
    public static function list(): void {
        ...
        if ($status && in_array($status, ['OPEN','KITCHEN','READY','PAID','PARTIAL','VOID','CANCELLED','REFUNDED'], true)) {
            $where[] = "o.status = ?"; $args[] = $status;
        }
        if ($branchId > 0) { $where[] = "o.branch_id = ?"; $args[] = $branchId; }
        ...
        $sql = "
          SELECT
            ...
            o.customer_id, COALESCE(o.customer_phone, c.phone) AS customer_phone
          FROM orders o
    ...
    public static function addItem($orderId): void {
        ...
        try {
            // EDIT allowed in OPEN + KITCHEN
            $o = $pdo->prepare("SELECT * FROM orders WHERE id=? AND status IN ('OPEN','KITCHEN') LIMIT 1");
            $o->execute([$orderId]);
            $order = $o->fetch();
    ...
    public static function setCharges($orderId): void {
        ...
        $pdo->beginTransaction();
        try {
            $u = $pdo->prepare("UPDATE orders SET delivery_fee=? WHERE id=? AND status IN ('OPEN','KITCHEN')");
    ...
    /** NEW: POST /api/orders/{id}/reopen (ADMIN/MANAGER) */
    public static function reopen($orderId): void {
        $user = Auth::requireAuth(['ADMIN','MANAGER']);
        $orderId = (int)$orderId;
        if ($orderId <= 0) json_error('order id required', 422);
        ...
        try {
            ...
            $status = strtoupper((string)$order['status']);
            if (in_array($status, ['OPEN','KITCHEN'], true)) {
                $pdo->commit();
                json_response(['reopened' => false, 'already' => true, 'status' => $status]);
                return;
            }
            if (!in_array($status, ['READY','PAID','PARTIAL'], true)) {
                json_error('Order cannot be reopened from status: '.$status, 422);
            }

            $u = $pdo->prepare("UPDATE orders SET status='OPEN', closed_at=NULL WHERE id=?");
            $u->execute([$orderId]);

            $pdo->commit();
            json_response(['reopened' => true, 'previous_status' => $status]);
        } catch (Throwable $e) {
            try { $pdo->rollBack(); } catch (Throwable $_) {}
            json_error('failed to reopen', 500, $e->getMessage());
        }
    }
